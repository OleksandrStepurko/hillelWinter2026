...
//
   ...
   public void testOneToOneMapping()
   {
      ...
      session.save( vehicle ); // --> v_id
      session.save( person ); // --> p_id + v_id
      session.getTransaction().commit();
      session.close();
...
//
   ...
   public void testDetachedObject()
   {
      ...
      person.setFirstName( "updated the name" ); // detach
...
//
   ...
   public void testCriteria()
   {
      ...
      criteria.add(
          Restrictions.or( Restrictions.eq( "name", "apple" ),
              Restrictions.between( "id", 4, 5 ) ) );
